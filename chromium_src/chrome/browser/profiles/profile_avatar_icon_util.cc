...
// This is currently only called when features::kNewProfilePicker is enabled.
std::unique_ptr<base::DictionaryValue> GetDefaultProfileAvatarIconAndLabel(
  ...
  return GetAvatarIconAndLabelDict(
      ...
      profiles::GetPlaceholderAvatarIndex(), selected,
      /*is_gaia_avatar=*/false);
#else
  return GetDefaultProfileAvatarIconAndLabel_ChromiumImpl(
      fill_color, stroke_color, selected);
#endif
}
