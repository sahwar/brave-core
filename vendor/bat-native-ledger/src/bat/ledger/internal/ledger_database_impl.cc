...
LedgerDatabaseImpl::LedgerDatabaseImpl(const base::FilePath& path)
    : db_path_(path), initialized_(false), use_temp_(false) {
  DETACH_FROM_SEQUENCE(sequence_checker_);
}

LedgerDatabaseImpl::LedgerDatabaseImpl(UseTempDatabaseForTesting)
    : db_path_(base::FilePath("")), initialized_(false), use_temp_(true) {
  DETACH_FROM_SEQUENCE(sequence_checker_);
}
...
void LedgerDatabaseImpl::RunTransaction(
  ...
  if (!db_.is_open()) {
    bool opened = use_temp_ ? db_.OpenTemporary() : db_.Open(db_path_);
    if (!opened) {
