...
class TestLedgerClient : public LedgerClient {
 public:
  ...
  std::list<TestNetworkResult>& network_results_for_testing() {
    return network_results_;
  }

  const std::list<TestNetworkResult>& network_results_for_testing() const {
    return network_results_;
  }

  const base::Value& state_store_for_testing() const { return state_store_; }

 private:
  type::DBCommandResponsePtr RunDBTransactionInTask(
